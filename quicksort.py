# Use a random number generator to fill up the given size of input array.
# Count and show the number of comparison and exchange operations.
# Have your own choice of the input data size.
...
# pivot_arr = [arr[0], arr[len(arr)- 1]]
# pivot = random.choice(pivot_arr)
...
# def quick_endsort(arr):
#     if len(arr) <= 1: # 정렬할 원소의 개수가 1보다 적다면 그냥 그 값 자체를 리턴해준다.
#         return arr
    
#     pivot, front = arr[len(arr) -1], arr[:len(arr) -1] # pivot을 마지막 숫자로 설정해 주고, front 값으로 나머지 리스트 값을 준다.
    
#     left = [x for x in front if x <= pivot] # left 라는 리스트를 만들어서 front리스트의 원소 값들을 pivot과 비교해 작으면 left에 넣어준다.
#     right = [x for x in front if x > pivot] # right 라는 리스트를 만들어서 front리스트의 원소 값들을 pivot과 비교해 크면 right에 넣어준다.
    
#     return quick_endsort(left) + [pivot] + quick_endsort(right) # 리턴 해준다.

# if(pivot == pivot_arr[0]):
#     arr = quick_sort(arr)
# else:
#     arr = quick_endsort(arr)

# print(arr)


# 2.3 The pivot is chosen as the median number, as suggested by Robert Sedgewick, among the first, the last, 
# and the mid elements between low and high. 
...
potential_pivot = [arr[0],arr[len(arr)//2],arr[len(arr) -1]]
sorted(potential_pivot)
pivot = potential_pivot[1]

def quick_startsort(arr):
    if len(arr) <= 1: # 정렬할 원소의 개수가 1보다 적다면 그냥 그 값 자체를 리턴해준다.
        return arr
    ...
    return quick_startsort(left) + [pivot] + quick_startsort(right) # 리턴 해준다.
...
def quick_midsort(arr):
    ...
    return quick_midsort(low) + equal + quick_midsort(high) # 정렬된 함수를 리턴해준다.


if(pivot == potential_pivot[0]):
    arr = quick_startsort(arr)
elif (pivot == potential_pivot[1]):
    arr = quick_midsort(arr)
else:
    arr = quick_endsort(arr)
